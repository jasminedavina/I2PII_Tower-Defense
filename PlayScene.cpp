...
const std::vector<int> PlayScene::code = { ALLEGRO_KEY_UP , ALLEGRO_KEY_DOWN, ALLEGRO_KEY_LEFT, ALLEGRO_KEY_RIGHT , ALLEGRO_KEY_LEFT, ALLEGRO_KEY_RIGHT, ALLEGRO_KEY_ENTER }; // CUSTOMIZE YOUR OWN KEY CHEAT
...
void PlayScene::Initialize() {
	...
	btn = new Engine::ImageButton("stage-select/dirt.png", "stage-select/floor.png", w - 250, h - 150, 200, 100);
	btn->SetOnClickCallback(std::bind(&PlayScene::PlayOnClick, this, 1));
	AddNewControlObject(btn);
	AddNewObject(new Engine::Label("Back", "pirulen.ttf", 36, w - 150, h - 100, 0, 0, 0, 255, 0.5, 0.5));
}
void PlayScene::PlayOnClick(int stage) {
...
void PlayScene::Update(float deltaTime) {
	...
	for (int i = 0; i < SpeedMult; i++) {
		...
		case 1:
			...
			break;
		case 0:
			EnemyGroup->AddNewObject(enemy = new DiceEnemy(SpawnCoordinate.x, SpawnCoordinate.y, 1 + rand() % 6));
			break;
		case 2:
			EnemyGroup->AddNewObject(enemy = new EnemyBoss(SpawnCoordinate.x, SpawnCoordinate.y, 6));
			break;
...
void PlayScene::OnMouseUp(int button, int mx, int my) {
	...
	const int y = my / BlockSize;
	if (button & 1) {
		if (mapState[y][x] != TILE_OCCUPIED && (preview->id() != 3) && (preview->id() != 4) && (preview->id() != 5)) {
			if (!preview)
				return;
			// Check if valid.
			...
			//mapStatus[y][x] = preview->GetPrice();
			// Purchase.
			...
			if (preview->id() == 1) {
				for (auto& n : TowerGroup->GetObjects()) {
					Turret* check = dynamic_cast<Turret*>(n);
					int halfWidth = check->GetBitmapWidth() / 2;
					int halfHeight = check->GetBitmapHeight() / 2;
					int xt = check->Position.x;
					int yt = check->Position.y;
					if (check->id() == 1) {
						if (((mx <= xt && mx >= xt - halfWidth) || (mx >= xt && mx <= xt + halfWidth)) && ((my <= yt && my >= yt - halfHeight) || (my >= yt && my <= yt + halfHeight))) {
							EarnMoney(-check->GetPrice());
							UIGroup->RemoveObject(preview->GetObjectIterator());
							TowerGroup->RemoveObject(check->GetObjectIterator());
							preview = new turretdouble(0, 0);
							xt = mx / BlockSize;
							yt = my / BlockSize; 
							
							// Construct real turret.
							preview->Position.x = xt * BlockSize + BlockSize / 2;
							preview->Position.y = yt * BlockSize + BlockSize / 2;
							preview->Enabled = true;
							preview->Preview = false;
							preview->Tint = al_map_rgba(255, 255, 255, 255);
							TowerGroup->AddNewObject(preview);
				...
				for (auto& n : TowerGroup->GetObjects()) {
					...
					int xt = check->Position.x;
					int yt = check->Position.y;
					if (check->id() == 0 || check->id() == 1 || check->id() == 2 || check->id()== -1) {
						if (((mx <= xt && mx >= xt - halfWidth) || (mx >= xt && mx <= xt + halfWidth)) && ((my <= yt && my >= yt - halfHeight) || (my >= yt && my <= yt + halfHeight))) {
							EarnMoney(-Shovel::Price);
							EarnMoney(check->GetPrice() / 2);
							UIGroup->RemoveObject(preview->GetObjectIterator());
							TowerGroup->RemoveObject(check->GetObjectIterator());
							xt = mx / BlockSize;
							yt = my / BlockSize;
				...
				for (auto& n : TowerGroup->GetObjects()) {
					...
					int yt = check->Position.y;
					int id = check->id();
					if (id == 0 || id == 1 || id == 2 || id == -1) {
						if (((mx <= xt && mx >= xt - halfWidth) || (mx >= xt && mx <= xt + halfWidth)) && ((my <= yt && my >= yt - halfHeight) || (my >= yt && my <= yt + halfHeight))) {
							EarnMoney(-Shifter::Price);
							//EarnMoney(check->GetPrice());
							UIGroup->RemoveObject(preview->GetObjectIterator());
							TowerGroup->RemoveObject(check->GetObjectIterator());
							xt = mx / BlockSize;
...
void PlayScene::OnKeyDown(int keyCode) {
	...
	if (keyCode == ALLEGRO_KEY_TAB) {
		...
		if (keyStrokes.size() == 7) {
			int i = 0;
			int flag = 1;
			for (std::list<int>::iterator it = keyStrokes.begin(); it != keyStrokes.end(); ++it) {
				if (*it != code[i++]) {
					flag = 0;
				}
			}
...
void PlayScene::ConstructUI() {
	...
	ConstructButton(2, "play/turret4.png", turret4::Price);
	ConstructButton(4, "play/Shifter.png", Shifter::Price);
	ConstructButton(5, "play/Spell.png", Spell::Price);
...
void PlayScene::ConstructButton(int id, std::string sprite, int price) {
	...
	btn = new TurretButton("play/floor.png", "play/dirt.png",
		Engine::Sprite("play/tower-base.png", 1294 + id % 3 * 76, 136 + (id/3) * 76, 0, 0, 0, 0),
		Engine::Sprite(sprite, 1294 + id % 3 * 76, 136 - 8 + (id/3) * 76, 0, 0, 0, 0)
		, 1294 + id % 3 * 76, 136 + (id/3) *76, price);
...
void PlayScene::UIBtnClicked(int id) {
	...
	if (id == 2 && money >= turret4::Price)
		preview = new turret4(0, 0);
	if (id == 5 && money >= Spell::Price)
		preview = new Spell(0, 0);
